  ...
  return (
    <>
      ...
      <div className="sticky top-[85px] z-40 border-y border-line bg-paper/95 backdrop-blur">
        <nav className="mx-auto flex max-w-content gap-6 overflow-x-auto px-6 py-3 text-base font-medium">
          {MODULE_NAV.map((item) => (
            <a
              key={item.href}
              href={item.href}
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-2">
            {AI_CAPABILITIES.map((c) => (
              <div key={c.title} className="glow-card rounded-lg bg-surface p-6">
                ...
                <p className="mt-2 text-base leading-relaxed text-slate">{c.desc}</p>
              </div>
            ))}
          </div>
          <div className="mt-8">
        ...
        <div className="mx-auto max-w-content px-6 py-14">
          ...
          <ul className="grid gap-4 sm:grid-cols-2">
            {SERVICE_LIST.map((item) => (
              <li key={item} className="flex gap-3 text-lg leading-relaxed text-slate">
