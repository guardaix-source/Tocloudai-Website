  ...
  return (
    <section className="hero-mesh">
      <div className="mx-auto max-w-content px-6 py-16 sm:py-20">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-ink sm:text-4xl">關於我們</h1>

        <p className="mt-8 max-w-2xl text-xl font-medium leading-relaxed text-ink">
          我們不做傳統設備販售導向的資安服務，專注為企業建置輕量、自動、有效、好維運的現代化資安防禦體系。
        </p>

        <div className="mt-10 max-w-2xl space-y-5 text-[15px] leading-relaxed text-slate">
          <p>
        ...
        <div className="mt-14 grid gap-5 sm:grid-cols-3">
          <div className="glow-card rounded-2xl bg-surface p-7">
            <p className="font-mono text-sm text-signal">輕量化</p>
            <p className="mt-2 text-[15px] leading-relaxed text-slate">高彈性、高 CP 值的導入模式，不綁死既有資源配置。</p>
          </div>
          <div className="glow-card rounded-2xl bg-surface p-7">
            <p className="font-mono text-sm text-signal">自動化</p>
            <p className="mt-2 text-[15px] leading-relaxed text-slate">AI 分擔人工審核負擔，聚焦真正需要人判斷的威脅。</p>
          </div>
          <div className="glow-card rounded-2xl bg-surface p-7">
            <p className="font-mono text-sm text-signal">可追溯</p>
            <p className="mt-2 text-[15px] leading-relaxed text-slate">每一起事件都有編號、時間軸與處置紀錄，全程可覆盤。</p>
          </div>
        </div>
      </div>
    </section>
